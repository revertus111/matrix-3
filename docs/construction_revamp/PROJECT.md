...
- Current user-prioritized side slice: Radial Worker Selection RWS-1 is RUNTIME VERIFIED; RWS-2 input/scaling foundation is runtime proven. The latest runtime clip showed edge A pinning improved but edge B still trailed the cursor because span length still came from screen-pixel calibration. RWS-2 now removes pixel-distance geometry entirely: A and B are Matrix3-resolved world tiles, center is their midpoint, and radius is half their exact world distance. Focused runtime acceptance remains before RWS-3.
