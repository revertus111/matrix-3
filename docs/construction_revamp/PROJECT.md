...
- Current user-prioritized side slice: Radial Worker Selection RWS-1 is RUNTIME VERIFIED; RWS-2 input/scaling foundation is runtime proven. Runtime feedback clarified that edge A still visually drifted backward, exposing a second issue: model scale was still based on an arbitrary percent-per-tile assumption. RWS-2 now derives scale from GFX 4171's real horizontal model bounds so rendered visual radius equals the A-to-midpoint world distance exactly; focused runtime acceptance remains before RWS-3. It remains client-only and does not change Bundle 2.2 server/runtime ownership.
