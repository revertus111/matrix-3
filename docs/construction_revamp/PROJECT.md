...
- Resume Here: runtime evidence from ops 9-11 exposed a render/input race: the scene-preview thread temporarily inserted the live gesture into the shared authoritative logical graph while the AWT mouse-release commit could read it. This produced impossible old/new topology transitions and made preview-only loops disappear on the following operation. Preview resolution is now snapshot-only and never mutates the authoritative graph. Post-release legacy A->B ghost rendering is removed so server-owned physical rails are the sole visual authority after release. Debug now preserves the actual gesture, stamps SNAPSHOT_PREVIEW_V2 + limits, and captures the applied world ~650 ms after queueing. Pull/rebuild, Clear Builds, then test nested loops/branches and >64 pieces. If the new log does not show SNAPSHOT_PREVIEW_V2 or still caps at 64, the running client is stale.
