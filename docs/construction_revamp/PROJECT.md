...
- IMPLEMENTED / NEEDS RUNTIME TEST under SAP AAA.
- Authoritative gameplay target is now Factorio-style rail construction: freely draw continuous track, make repeated 90-degree turns, extend existing track, begin a new drag from the middle of authored track to create a branch, connect networks later, and eventually resolve degree-3/4 nodes to proper switch/splitter/junction art.
- The rejected route abstractions (A->B continuation, movable-B whole-route recalculation, endpoint-only authoredPath extension) are SUPERSEDED for new commits. Do not resume those experiments unless needed only for cleanup/removal.
- Rail Network V1 authors a logical tile topology in the current client session. N/E/S/W adjacency is an internal resolver detail only: two opposite neighbors resolve straight, two perpendicular neighbors resolve the accepted three-piece curve, and degree-3/4 nodes are retained logically with temporary straight-through art until final junction assets exist.
- Physical RS3 objects are derived output. Every network edit resolves the complete logical graph deterministically and submits old physical set -> new physical set through the existing packet-safe atomic server replacement. This avoids incremental curve/seam ownership mutation.
