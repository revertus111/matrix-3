...
- Resume Here: pull and runtime-test the authored-edge + capacity correction in one session. Keep Build Palette Debug OFF normally; enable it only if a failure reproduces. Debug export is now pinned to Client/data/construction regardless of whether Eclipse launches from repo root or Client, and the existing palette button visibly shows Dbg ON/OFF. If this gate passes, next patch is server-persistent logical rail graph + reconstruction on settlement entry, followed by junction/switch visual asset resolution.
