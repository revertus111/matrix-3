...
- A->B Rail Route Preview V2 is IMPLEMENTED / NEEDS RUNTIME TEST. The bend prefers the first saved CURVE layout and rotates both each component model and its relative X/Y offset for the four corner quadrants; the old single-object curve remains fallback only.
