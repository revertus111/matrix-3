...
- Resume Here: pull and runtime-test the authored-edge + capacity correction in one session. Keep Build Palette Debug OFF normally; enable it only if a failure reproduces. If this gate passes, next patch is server-persistent logical rail graph + reconstruction on settlement entry, followed by junction/switch visual asset resolution.
